...
def price_distribution_binned_interactive(market_data):
    ...
    if event.selection.points:
        ...
        if selected_range not in ["Free", "Not Available"]:
            ...
            price_point_counts = (
                selected_games["initial_price"]
                .value_counts()
                .sort_index()
                .reset_index()
            )

            price_point_counts.columns = [
                "Price",
                "Games"
            ]
            ...
            second_figure = px.bar(
                ...
                x="Price",
                y="Games",
                title=f"Price Points Within {selected_range}",
                labels={
                    "Price": "Price Point",
                    "Games": "Number of Games"
                }
            ...
            st.info(
                f"{selected_range} does not contain individual "
                "paid price points."
            )
...
def price_histogram_interactive(market_data):
    ...
    # Separate Free, Not Available, and paid prices
    free_data = price_counts[
        price_counts["Price"] == "Free"
    ]

    not_available_data = price_counts[
        price_counts["Price"] == "Not Available"
    ]

    paid_data = price_counts[
        ~price_counts["Price"].isin(
            ["Free", "Not Available"]
        )
    ].copy()

    paid_data["sort_price"] = (
        paid_data["Price"]
    ...
    # Create pricing status pie chart
    status_figure = px.pie(
        status_counts,
        names="Status",
        values="Games",
        title="Pricing Status Among Similar Games"
    )
    ...
    with col1:
        ...
        event = st.plotly_chart(
            ...
            on_select="rerun",
            selection_mode="points",
            key="price_histogram_interactive"
        )

        # Check if a bar was clicked
        if event.selection.points:

            selected_price = event.selection.points[0]["x"]

            selected_games = price_data[
                price_data["Price"] == selected_price
            ].copy()

            st.subheader(
                f"Games at {selected_price}"
            )

            st.caption(
                f"{len(selected_games)} similar games "
                f"are listed at this price."
            )

            # Display only the information needed
            ...
            selected_games = selected_games.rename(
                columns={
                    "original_name": "Game",
        ...
        st.caption(
            "Shows the number of similar paid games at each "
            "listed price point. Click a bar to explore the "
            "games at that price."
        )
